...
namespace LogicLayer
{
    public class UC4M4_SaveData : ISave
    {
        ...
        public UC4M4_SaveData(IData datalayer, DataTreatment data)
        {
        ...
        public void SaveDataLogic(string IDno, string Procedure, string CPRno, string name, DateTime timeAndDate) //, byte[] bloodpressureList, double Calibrate) //
        {
            ...
            List<double> localList = dataTreatment_.GetFullList();
            datalayerInterface.SaveInDatabase(IDno, Procedure, CPRno, name, timeAndDate, localList); // Lav GetFullList i datatreatment
        }
